...
#' @description This package provides implementation of the network-based variable selection method proposed in Ren et al (2017) and the robust network-based method for survival response in Ren et al (2019). In addition to network penalty, regnet also allows users to use classical MCP or LASSO penalty.
#'
...
#' There are three arguments control the fitting method
#' \tabular{rl}{
...
#' robust: \tab whether to use robust methods for modeling (Robust methods \cr \tab are only available for survival response for now).
#' }
...
#' }
#'
#' @seealso \code{\link{cv.regnet}} \code{\link{regnet}}
NULL
